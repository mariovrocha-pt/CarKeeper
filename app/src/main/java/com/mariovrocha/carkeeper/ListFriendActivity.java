...
import com.mariovrocha.carkeeper.Utility.PlacesUtils;
import com.mariovrocha.carkeeper.Utility.PolylineUtils;
import com.mariovrocha.carkeeper.ViewHolder.FriendViewHolder;
...
public class ListFriendActivity extends BaseActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!EasyPermissions.hasPermissions(this, perms)){
            EasyPermissions.requestPermissions(this, "Asking for location",101,perms);
        }
    ...
    private void doMeet(final String uid, final String friendName){
        final ProgressDialog progressDialog = makeRawProgressDialog("Waiting for an answer");
        ...
        final CountDownTimer timer = new CountDownTimer(30000, 1000) {
            ...
            public void onFinish() {
                ...
                rootRef.child("invite").child(uid).removeValue();
                Toast.makeText(getApplicationContext(), friendName + " is not answering", Toast.LENGTH_LONG).show();
            }
        };
        timer.start();
        rootRef.child("invite").child(uid).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.getValue() != null) {
                    ...
                    progressDialog.dismiss();
                    timer.cancel();
                    Toast.makeText(getApplicationContext(), friendName + " did not receive an invitation", Toast.LENGTH_LONG).show();
                }
            }
            @Override
            public void onCancelled(DatabaseError databaseError) {}
                    ...
                    rootRef.child("users").child(uid).addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            if(dataSnapshot.child("LAT").getValue()!=null&&dataSnapshot.child("LONG").getValue()!=null){
                                ...
                                holder.mTvLastLoc.setText("Last location not found.");
                            }
                        }
